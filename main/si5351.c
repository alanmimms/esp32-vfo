...
// VCO frequency range (Hz)
static const uint32_t vcoMax = MHZ(900);
static const uint32_t vcoMin = MHZ(600);
// static const uint32_t vcoMid = (vcoMax + vcoMin) / 2; // Unused currently
...
static void writeRegs(RegisterNumber_t regBase, const uint8_t *bufP, uint8_t len) {
  // Ensure i2cDevH is valid before use
  if (!i2cDevH) {
      // Log error or handle appropriately
      // ESP_LOGE("SI5351", "I2C device handle not initialized");
      return;
  }
  uint8_t baseAddr = (uint8_t)regBase; // Cast enum to uint8_t for transmission

  // Using ESP-IDF i2c_master_transmit which handles address phase internally
  esp_err_t ret = i2c_master_transmit(i2cDevH, &baseAddr, 1, (uint8_t*)bufP, len, -1); // Use TickType_t for timeout
  ESP_ERROR_CHECK(ret); // Check the return value
}

// Write a single byte 'value' to register 'regAddr'.
static void writeReg(RegisterNumber_t regAddr, uint8_t value) {
    writeRegs(regAddr, &value, 1);
}


// Read len bytes from consecutive registers starting at regBase into bufP.
static void readRegs(RegisterNumber_t regBase, uint8_t *bufP, uint8_t len) {
  // Ensure i2cDevH is valid before use
  if (!i2cDevH) {
      // Log error or handle appropriately
      // ESP_LOGE("SI5351", "I2C device handle not initialized");
  ...
  // Using ESP-IDF i2c_master_transmit_receive
  ESP_ERROR_CHECK(i2c_master_transmit_receive(i2cDevH, &baseAddr, 1, bufP, len, -1)); // Use TickType_t for timeout
}

// Read a single byte from register 'regAddr'.
static uint8_t readReg(RegisterNumber_t regAddr) {
    uint8_t value;
...
static void setMSynthParams(RegisterNumber_t synthBase, uint32_t p1, uint32_t p2, uint32_t p3, uint8_t rDivEncoded, bool divBy4) {
  uint8_t params[8]; // 8 registers for Multisynth parameters

  // Register 0 (base+0): MSx_P3[15:8]
  params[0] = (p3 >> 8) & 0xFF;
  // Register 1 (base+1): MSx_P3[7:0]
  params[1] = p3 & 0xFF;
  // Register 2 (base+2): R_DIV[2:0], MSx_DIVBY4[1:0], MSx_P1[17:16]
  // Use uint8_t rDivEncoded directly
  params[2] = rDivEncoded | ((p1 >> 16) & MS_P1_HIGH_MASK) | (divBy4 ? MS_DIVBY4_ON : MS_DIVBY4_OFF);
  // Register 3 (base+3): MSx_P1[15:8]
  params[3] = (p1 >> 8) & 0xFF;
  // Register 4 (base+4): MSx_P1[7:0]
  params[4] = p1 & 0xFF;
  // Register 5 (base+5): MSx_P3[19:16], MSx_P2[19:16]
  params[5] = ((p3 >> 12) & 0xF0) | ((p2 >> 16) & 0x0F);
  // Register 6 (base+6): MSx_P2[15:8]
  params[6] = (p2 >> 8) & 0xFF;
  // Register 7 (base+7): MSx_P2[7:0]
  params[7] = p2 & 0xFF;
...
// 'rDivEncoded' parameter is now uint8_t
static void setMSynthFrac(RegisterNumber_t synthBase, uint32_t a, uint32_t b, uint32_t c, uint8_t rDivEncoded) {
  uint32_t p1, p2, p3;
...
// Calculates P1, P2, P3 and calls setMSynthParams.
// 'rDivEncoded' parameter is now uint8_t
static void setMSynthInt(RegisterNumber_t synthBase, uint32_t div, uint8_t rDivEncoded) {
  uint32_t p1, p2, p3;
  bool divBy4 = false;
...
static void configurePLL(RegisterNumber_t pllBase, uint32_t vcoFreq) {
    ...
    RegisterNumber_t ctrlRegAddr = (pllBase == REG_MSNA_BASE) ? REG_CLK6_CONTROL : REG_CLK7_CONTROL;
...
static void configureOutputSynth(uint8_t clkNumber, uint32_t outputFreq, uint32_t vcoFreq, bool usePLLA, uint8_t drive) {
    RegisterNumber_t msBase;
    uint8_t rDiv = 1;
    uint8_t rDivEncoded = R_DIV_1; // Use uint8_t
    RegisterNumber_t clkControlReg = (RegisterNumber_t)(REG_CLK_CONTROL_BASE + clkNumber);
    ...
    // Clear fields first
    clkCtrlVal &= ~(CLK_PDN | MS_INT | MS_SRC_PLLB | CLK_INV | CLK_SRC_MASK | CLK_IDRV_MASK); // Note: MS_SRC_PLLA is 0, so clearing MS_SRC_PLLB sets it to PLLA
    // Set fields
...
void SI5351Init() {
  ...
  for (uint8_t clk = 0; clk < nClocks; ++clk) {
    writeReg((RegisterNumber_t)(REG_CLK_CONTROL_BASE + clk), CLK_PDN); // Set only power down bit
  }
...
void SI5351SetDrive(uint8_t clkNumber, uint8_t drive) {
    ...
    RegisterNumber_t regAddr = (RegisterNumber_t)(REG_CLK_CONTROL_BASE + clkNumber);
...
void SI5351SetFreq(uint8_t clkNumber, uint32_t f) {
  ...
  if (f == 0) {
    ...
    RegisterNumber_t ctrlReg = (RegisterNumber_t)(REG_CLK_CONTROL_BASE + clkNumber);
  ...
  uint32_t vcoFreq = MHZ(900); // Target VCO frequency (fixed for simplicity)
  bool usePLLA = true;         // Use PLLA for this clock (fixed for simplicity)
  RegisterNumber_t pllBase = usePLLA ? REG_MSNA_BASE : REG_MSNB_BASE;
...
void SI5351SetFreqQuadrature(uint32_t f01, bool invertPhase) {
    ...
    // --- Frequency Planning ---
    uint32_t vcoFreq = MHZ(900); // Use a fixed VCO for simplicity
    bool usePLLA = true;         // Use PLLA for both clocks
    RegisterNumber_t pllBase = REG_MSNA_BASE;
    ...
    RegisterNumber_t regAddr0 = (RegisterNumber_t)(REG_CLK_PHASE_OFFSET_BASE + 0);
    RegisterNumber_t regAddr1 = (RegisterNumber_t)(REG_CLK_PHASE_OFFSET_BASE + 1);

    if (invertPhase) {
        // Set CLK0 offset, CLK1 zero
        writeReg(regAddr0, phaseOffsetRegVal);
        writeReg(regAddr1, 0);
    } else {
        // Set CLK1 offset, CLK0 zero
        writeReg(regAddr0, 0);
        writeReg(regAddr1, phaseOffsetRegVal);
    }
